    ...
    ILogger<QdrantKnowledgeClient> logger) : IQdrantKnowledgeClient
{
    ...
    public async Task<IReadOnlyList<TemplateKnowledgeState>> GetTemplateStatesAsync(
        CancellationToken cancellationToken = default)
    {
        var states = new List<TemplateKnowledgeState>();
        JsonElement? offset = null;
        ...
        do
        {
            ...
            foreach (var point in GetRequiredProperty(result, "points").EnumerateArray())
            {
                ...
                if (!TryReadGuid(point, "id", out var pointId)
                    ...
                    || !TryReadString(pointPayload, "contentHash", out var contentHash))
                {
                    throw new AiProviderException("Qdrant returned an invalid Template point.");
                }

                states.Add(new TemplateKnowledgeState(
                    pointId,
                    templateId,
                    sourceVersion,
                    chunkId,
    ...
    public async Task UpsertTemplatePointsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation(
            "Qdrant Template knowledge synchronized: {UpsertedCount} points upserted",
            points.Count);
    }

    public async Task DeleteTemplatePointsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation(
            "Qdrant Template knowledge synchronized: {DeletedCount} stale points deleted",
            pointIds.Count);
    }

    public async Task<IReadOnlyList<TemplateKnowledgeCandidate>> SearchTemplateAsync(
        float[] queryVector,
        Guid templateId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var response = await SendAsync(
            ...
            new
            {
                ...
                filter = new
                {
                    must = new object[]
                    {
                        new { key = "sourceType", match = new { value = "Template" } },
        ...
        var points = GetRequiredProperty(
            ...
            "points");
        var candidates = new List<TemplateKnowledgeCandidate>();
        ...
        foreach (var point in points.EnumerateArray())
        {
            ...
            candidates.Add(new TemplateKnowledgeCandidate(
                pointId,
                returnedTemplateId,
