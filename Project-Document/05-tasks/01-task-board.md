...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ------ | -------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T0-00 | Chốt quyết định AI RAG: LLM provider/model, embedding, vector store, nguồn tri thức, guardrail, SLO/cost, evaluation                      | `03-ai-rag-design.md` mục 6 | `[x]`  | P0       | Không      | Project Owner là AI owner/người duyệt; fixture đủ 45 ca; toàn bộ safety/quality/SLO/RAM gate và chấm 9 draft đạt; session log có model digest, `MinScore`, cold/warm metric và ngày hiệu lực; tài liệu chuyển **Approved for MVP/demo**. Không đổi public API hoặc EF schema. **Chốt hạn: trước T2-04, T3-02 và T3-03.** |
...
| T2-04 | AI gợi ý và xác nhận điều phối (API + UI SCR-009)                                             | FR-009   | `[x]`  | P0       | T0-00, T2-02 | AI service gọi qua interface/typed client; lỗi/timeout AI không đổi assignment (trả `503`); Văn thư luôn là người xác nhận cuối; test AI lỗi không mutation. |
...
| 2026-08-01 | T0-00 | Handoff host `LAPTOP-A07DUJIR` đạt static checks và đã xác minh Ollama archive, nhưng RAM preflight chỉ 8.499 GB (< 9 GB operational gate); benchmark chưa được khởi động. | Giữ `[~]`/Draft; giải phóng RAM rồi tạo session log mới và chạy lại đủ 45 ca, không dùng metric partial hoặc nới ngưỡng. |
| 2026-08-01 | T0-00 | Supplemental trial đã chạy đủ 45 ca với preflight capture 8.423 GB; retrieval đạt nhưng schema 72.73%, assignment 50%, draft 0/9, review 10/12 và SLO thất bại. | Giữ `[~]`/Draft; raw result hash `1c7e7008...`; không dùng metric trial để phê duyệt, tiếp tục cần lượt Official với preflight >= 9 GB. |
| 2026-08-01 | T0-00 | Runner candidate T0-00-RAG-MVP-20260801-v2 đã self-test đạt; Official preflight hiện chỉ 7.437 GB RAM khả dụng (< 9 GB), workload chưa khởi động. | Giữ [~]/Draft; không bypass ngưỡng hoặc sinh evidence giả; giải phóng RAM rồi chạy lại wrapper v2 và đủ 45 ca. |
